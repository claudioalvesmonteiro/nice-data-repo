...
##### O formato mais universal para salvar um arquivo dataframe em forma de tabela é `.csv`. Salvamos com um separador em ';' para ele ser legível ao EXCEL e desativamos o padrão de salvar com os nomes das linhas (geralmente desnecessario).

</br> 

```{r}
write.table(dados_altura, 
            'dados_altura.csv', 
            sep = ';', 
            row.names = FALSE)
```
...
- Entre no portal [Dados Abertos Recife](http://dados.recife.pe.gov.br/dataset/acidentes-de-transito-com-e-sem-vitimas) e baixe a base da CTTU para os acidentes de trânsito em Recife, no ano 2016;
- Mova o banco para sua pasta de trabalho;
- Carregue o banco no R.
