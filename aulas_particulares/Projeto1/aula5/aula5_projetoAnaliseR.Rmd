...
Agora vamos adentrar no mundo da visualiza??o espacial em R. Como j? sabemos, o R conta com diversos pacotes para performar uma série de an?lises. Aqui vamos trabalhar com alguns pacotes de manipula??o de dados geogr?ficos e com o pacote **ggplot2** para a visualiza??o.

Para visualizar informações distribu?das no espaço devemos primeiro ter acesso a uma base cartogr?fica do espaço que queremos projetar essas informações. Por exemplo, se queremos visualizar a distruibui??o da popula??o brasileira no mapa dos estados precisamos de um arquivo com o **desenho cartogr?fico** do Brasil e os *pol?gonos* de cada Estado. Esse arquivos podem ter os formatos **.geoJSON**, **.CSV**, **.KML**, **.SHP**, entre outros.

Hoje vamos utilizar o formato **.SHP**, também chamado de **shapefile**. Acesse o link [aqui](https://github.com/claudioalvesmonteiro/municipality-inovation/tree/master/Original%20Data/Geodata) e baixe todos os arquivos com "estados_2010" para seu diret?rio de trabalho.

## Importando o arquivo

Para importar o arquivo instale o pacote "raster", bem como suas dependências. Neste pacote h? a fun??o *shapefile()*, que funciona como a *read.csv()*, mas para arquivos do tipo **.SHP**.
...
## Visualiza??o do Shape

Para produzir um gr?fico espacial do tipo ggplot precisamos converter o objeto que importamos como um `SpatialPolygonsDataFrame`, uma base de dados com informações georreferenciais, para um tipo de dados `dataframe`.
...
# combinar duas bases
AtlasBrasil$codigo_ibg <- as.character(AtlasBrasil$C?digo)
...
# An?lise/Minera??o de Texto em R
...
## Tratar//padronizar textos para an?lise
